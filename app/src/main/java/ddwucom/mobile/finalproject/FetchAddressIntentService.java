...
public class FetchAddressIntentService extends IntentService {
    ...
    @Override
    protected void onHandleIntent(@Nullable Intent intent) { //main에서 startService할 때 넣어준 intent
        //지오코더 생성
        geocoder = new Geocoder(getApplicationContext(), Locale.getDefault()); //locale >> 한글로 설정
...
//        MainActivity 가 전달한 Intent 로부터 위도/경도와 Receiver 객체 설정
        ...
        receiver = intent.getParcelableExtra(Constants.RECEIVER); //parcelable 자료형

        List<Address> addresses = null;

//        위도/경도에 해당하는 주소 정보를 Geocoder 에게 요청
        try {
            addresses = geocoder.getFromLocation(latitude, longitude, 1); //결과의 개수 입력
        } catch (IOException e) {
            e.printStackTrace();
...
//        결과로부터 주소 추출
    ...
    private void deliverResultToReceiver(int resultCode, String message) {
        ...
        receiver.send(resultCode, bundle); //코드값과 bundle을 보내준다.
    }
